...
typedef struct PaintSurfacePoint {
	...
	*/
	float color[3];
	float alpha;
	float depth;	// for displacement map

	/*
	*	Effect / moving layer data
	*	! Only generated if effects enabled !
	*/
	int neighbours[8];	// Indexes of 8 neighbour pixels if exist
	float neighbour_dist[8];	//	Distances to all 8 neighbour pixels							
	...
	*/
	...
	int neighbour_pixel;	// If this pixel isn't on a face, this is the pixel
							// whose neighbour it is
	short quad;
	...
	float invNorm[3];  // current pixel world-space inverted normal. includes smooth+flat faces
