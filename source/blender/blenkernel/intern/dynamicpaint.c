			...
			// Ignore Sticky
				...
				*/
				textured_face_get_uv(co, normal, uv, faceIndex, isQuad, tface);
							...
							TEXCO_WINDOW, TEXCO_STRAND, TEXCO_STRESS
							*/
...
/*
*	Get material+textures diffuse color in given coordinates
*/
...
*/
int DynamicPaint_PaintMesh(DynamicPaintCanvasSettings *canvas, Vec3f *canvasVerts, DynamicPaintPainterSettings *paint, Object *canvasOb, Object *paintOb, float timescale)
{
	DerivedMesh *dm = NULL;
	MVert *mvert;
	MFace *mface;
				...
				if (i >= 0)
				{
					...
					for (ss=0; ss<canvas->surface->pixelSamples; ss++) {
						...
						if ((paint->collision == MOD_DPAINT_COL_DIST || paint->collision == MOD_DPAINT_COL_VOLDIST) && (!hit_found))
						{
							float proxDist = -1.0f;
							/*
							*	If pure distance proximity, find nearest point
							*/
							...
							// If a hit was found get required values
							if (proxDist >= 0.0f) {
...
*/
int DynamicPaint_PaintParticles(DynamicPaintCanvasSettings *canvas, ParticleSystem *psys, DynamicPaintPainterSettings *paint, Object *canvasOb, float timescale)
{
	int yy;
	int tWidth = canvas->surface->w;
	int tHeight = canvas->surface->h;
			...
			if (i >= 0)
			{
					...
					if (paint->flags & MOD_DPAINT_DO_DISPLACE) {
						...
						*/
						...
						// take a sqrt of distance to get displace depth
						nearest.dist = sqrt(nearest.dist / radius) * radius;
						
						// get displace depth
						sdepth = (radius - nearest.dist) / normal_scale * 2.0f;
