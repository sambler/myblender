...
EdgeHash *BLI_edgehash_new(void) {
	...
	eh->buckets= MEM_mallocN(eh->nbuckets*sizeof(*eh->buckets),"BLI_edgehash_new-buckets");
...
void BLI_edgehash_insert(EdgeHash *eh, int v0, int v1, void *val) {
	unsigned int hash;
	Entry *e= MEM_mallocN(sizeof(*e),"BLI_edgehash_insert");
	...
	if (++eh->nentries>eh->nbuckets*3) {
		...
		eh->nbuckets= hashsizes[++eh->cursize];
		eh->buckets= MEM_callocN(eh->nbuckets*sizeof(*eh->buckets),"BLI_edgehash_insert-buckets");
...
EdgeHashIterator *BLI_edgehashIterator_new(EdgeHash *eh) {
	EdgeHashIterator *ehi= MEM_mallocN(sizeof(*ehi),"BLI_edgehashIterator_new");
	ehi->eh= eh;
	ehi->curEntry= NULL;
	ehi->curBucket= -1;
	while (!ehi->curEntry) {
