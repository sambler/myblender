...
static void rna_def_dynamic_paint_painter_settings(BlenderRNA *brna)
{
	...
	*/
	...
	RNA_def_property_ui_text(prop, "Paint Wetness", "Paint Wetness. Visible in wet map. Effects only affect wet paint.");
	RNA_def_property_update(prop, 0, NULL);

	prop= RNA_def_property(srna, "paint_erease", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flags", MOD_DPAINT_EREASE);
	RNA_def_property_ui_text(prop, "Erease Paint.", "Makes this object to erease paint instead of adding it.");
	RNA_def_property_update(prop, 0, NULL);
	...
	*/
	...
	RNA_def_property_ui_text(prop, "Face Aligned", "Check proximity in normal direction only.");
	RNA_def_property_update(prop, 0, NULL);
